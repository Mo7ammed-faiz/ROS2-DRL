...
class DqnAgent(Node):
    ...
    def finish_episode(self, step, eps_duration, outcome, dist_traveled, reward_sum, loss_critic, lost_actor):
            ...
            self.episode += 1
            display_ep = self.episode if self.training else (self.episode - self.load_episode)
            limit = MAX_EPISODES if self.training else TEST_EPISODES
            print(
                f"Episode {display_ep}/{limit} | "
                f"Reward: {reward_sum:>7.0f} | "
                f"Outcome: {self.translate_outcome(outcome):<10} | "
                f"Steps: {step:<5} | TotalSteps: {self.total_steps:<7} | "
                f"Time: {eps_duration:>6.2f}s"
            )

            if (not self.training):
                self.logger.update_test_results(step, outcome, dist_traveled, eps_duration, 0)
                self.test_outcomes.append(outcome)
                if display_ep >= TEST_EPISODES:
                    counts = Counter(self.test_outcomes)
                    print(f"\n=== Test complete: {TEST_EPISODES} episodes on session '{self.sm.session}' (loaded ep {self.load_episode}) ===")
                    for k in sorted(counts.keys()):
                        pct = 100.0 * counts[k] / TEST_EPISODES
                        print(f"  {self.translate_outcome(k):<12}: {counts[k]:>3}  ({pct:5.1f}%)")
                    rclpy.shutdown()
                    sys.exit(0)
                return
            ...
            if self.training and self.episode >= MAX_EPISODES:
                if self.episode % 100 != 0:
                    self.sm.save_session(self.episode, self.model.networks, self.graph.graphdata, self.replay_buffer.buffer)
                    self.sm.store_model(self.model)
                    self.logger.update_comparison_file(self.episode, self.graph.get_success_count(), self.graph.get_reward_average())
                print(f"Reached MAX_EPISODES={MAX_EPISODES}. Training complete. Final session: {self.sm.session}")
                rclpy.shutdown()
                sys.exit(0)
...
#Actor Class — Dueling DQN architecture.
#Shared feature trunk feeds two heads: state-value V(s) and advantage A(s,a).
#Final Q(s,a) = V(s) + (A(s,a) - mean_a A(s,a)) — better when many actions yield similar value.
class Actor(nn.Module, ABC):
    ...
    def forward(self, states, visualize=False):
        ...
        v = self.value_head(x2)
        a = self.advantage_head(x2)
        action = v + (a - a.mean(dim=-1, keepdim=True))
...
class DQN(ABC):
    ...
    #Function to train the network, this function trains the network using the replay buffer (Double DQN)
    def train(self, state, action, reward, state_next, done):
        #Double DQN: online net selects best next action, target net evaluates it.
        #This decouples action selection from value estimation, reducing Q overestimation bias.
        action = torch.unsqueeze(action, 1)
        with torch.no_grad():
            next_actions = self.actor(state_next).argmax(1, keepdim=True)
            Q_next = self.actor_target(state_next).gather(1, next_actions)
